...
import { creditRetainerPayment } from "@/lib/retainer-credit";
...
export async function POST(req: Request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // Retainer period paid → record the payment + roll next-due forward (idempotent).
    if (dbConfigured && retainerId) {
      await creditRetainerPayment(retainerId, String(session.payment_intent ?? ""));
    }
